...
client.connect(err => {
  ...
  //updating a seeking
  app.patch("/update/seeking/:id", (req,res) => {
    console.log(req.body);
    const id = ObjectID(req.params.id);
    //be careful with the $set
    seekingCollection.updateOne({_id: id}, {
        $set: {name: req.body.name, description: req.body.description, location: req.body.location, categories: req.body.categories, delivery: req.body.delivery}
    })
    .then(result => res.send(result))
  });
  ...
  //delete a seeking post
  app.delete("/delete/seeking/:id", (req,res) => {
    const id = ObjectID(req.params.id);
    seekingCollection.deleteOne({_id: id})
    .then(documents => res.send(!!documents.value))
  })
